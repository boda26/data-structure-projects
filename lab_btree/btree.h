...
size_t search(const std::vector<T>& elements, const C& val, unsigned start, unsigned end) {
    if (end - start == 0) {
        return end - 1;
    }
	unsigned int middle = ((end + start) / 2);
	if (elements[middle] > val) {
		return search(elements, val, 0, middle);
	}
	if (elements[middle] < val) {
		return search(elements, val, middle + 1, end);
	}
	else {return middle;}
}
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    size_t a = 0;
    while(a < elements.size() && val > elements[a]) {
        a++;
    }
