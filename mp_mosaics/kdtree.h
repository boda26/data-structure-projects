...
class KDTree
{
  ...
  private:
     ...
     */
    unsigned partition(vector<Point<Dim>>& list, int dimension, unsigned l, unsigned r, unsigned index);
    
    Point<Dim>& quickSelect(vector<Point<Dim>>& list, int dimension, unsigned l, unsigned r, unsigned k);

    KDTreeNode* build(vector<Point<Dim>>& list, int dimension, unsigned l, unsigned r);

    void copy_(KDTreeNode* subroot, KDTreeNode* other);

    void clear_(KDTreeNode* subroot);

    Point<Dim> nearestNeighbor(const Point<Dim>& query, int dimension, KDTreeNode* subroot) const;
};
