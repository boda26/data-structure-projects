...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    for (unsigned i = 0; i < frequencies.size(); i++) {
        HuffmanTree::TreeNode* a = new HuffmanTree::TreeNode(frequencies[i]);
        singleQueue.push(a);
    }
    while (singleQueue.size() + mergeQueue.size() > 1) {
        HuffmanTree::TreeNode* first = HuffmanTree::removeSmallest(singleQueue, mergeQueue);
        HuffmanTree::TreeNode* second = HuffmanTree::removeSmallest(singleQueue, mergeQueue);
        HuffmanTree::TreeNode* internal = new HuffmanTree::TreeNode(first->freq.getFrequency() + second->freq.getFrequency());
        internal->left = first;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (bfile.getNextBit() == true) {
        node = new HuffmanTree::TreeNode(Frequency(bfile.getNextByte(), 0));
        return node;
    } else {
        node = new HuffmanTree::TreeNode(0);
